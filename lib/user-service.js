...
export async function createUser(input, uploadMetadata) {
  const data = normalizeRegisterInput(input);
  if (data.role === ROLES.EQUIPE && process.env.ALLOW_ADMIN_REGISTER !== "true") {
  ...
  return withTransaction(async (client) => {
    ...
    if ([ROLES.EQUIPE, ROLES.COORDENADORES, ROLES.LIDERES].includes(data.role) && !rootLeaderId) {
    ...
    if ([ROLES.EQUIPE, ROLES.COORDENADORES, ROLES.LIDERES].includes(data.role)) {
      leaderProfile = await ensureLeaderProfile(client, user.id, config.appUrl);
    }
