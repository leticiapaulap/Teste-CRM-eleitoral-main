...
import { makeInternalEmail, withPublicEmail } from "./email.js";
import { canLogin, hashPassword, ROLES } from "./security.js";
...
import { makeReferralCodeForRole, makeReferralUrl, buildTree } from "./referrals.js";
...
export function normalizeRegisterInput(input) {
  ...
  return {
    ...
    email: canLogin(role) ? validateEmail(input.email) : validateOptionalEmail(input.email),
    phone: validatePhone(input.phone || input.telefone || input.whatsapp),
    password: canLogin(role)
      ? validatePassword(input.password || input.senha)
      : validatePassword(input.password || input.senha || randomBytes(12).toString("hex")),
    role,
    photoUrl: assertValidPhotoUrl(input.photoUrl || input.photo_url, { required: false }) || null,
    ...
    latitude,
    longitude,
    referralCode: normalizeReferralCode(input.referralCode || input.referral_code || input.ref),
    consentAccepted: validateBoolean(input.consent_accepted ?? input.consentimento ?? input.aceite_lgpd, "consent_accepted"),
  };
}

export async function createUser(input, uploadMetadata, { allowAdminRole = false, appUrl, parentUserId = null, actorUserId = null } = {}) {
  ...
  return withTransaction(async (client) => {
    ...
    if (data.referralCode || parentUserId) {
      const referral = data.referralCode
        ...
        : await client.query(
          `select u.id as user_id, nn.root_leader_id, nn.level
             from users u
      ...
      if (!parentUser) throw new ApiError(400, data.referralCode ? "Codigo de indicacao invalido." : "Responsavel do cadastro invalido.");
    ...
    const userResult = await client.query(
      ...
      [data.name, data.email || makeInternalEmail(data.phone || data.name), data.phone, passwordHash, data.role, data.photoUrl]
    );
    ...
    if ([ROLES.EQUIPE, ROLES.COORDENADORES, ROLES.LIDERES, ROLES.CADASTRADOS].includes(data.role)) {
      leaderProfile = await ensureLeaderProfile(client, user.id, appUrl || config.appUrl, data.role);
    }

    await client.query(
      "insert into audit_logs (actor_user_id, action, target_user_id, metadata) values ($1, $2, $3, $4)",
      [actorUserId || user.id, "USER_REGISTERED", user.id, JSON.stringify({ role: data.role, referralCode: data.referralCode, parentUserId })]
    );

    return { user: withPublicEmail(user), leaderProfile };
  });
}

export async function ensureLeaderProfile(clientOrPool, userId, appUrl, role = "") {
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    const code = makeReferralCodeForRole(role);
...
export async function getLeaderNetwork(rootLeaderId, { page = 1, limit = 50, tree = false, filters = {} } = {}) {
  ...
  if (tree) {
    ...
    return { items: buildTree(result.rows.map(withPublicEmail)) };
  }
  ...
  return { items: paged.rows.map(withPublicEmail), page: Number(page), limit: Number(limit), total: count.rows[0].total };
}
