...
export function normalizeRegisterInput(input) {
  ...
  const photoRequired = ![ROLES.DEPUTADO, ROLES.EQUIPE].includes(role);
  return {
...
export async function createUser(input, uploadMetadata) {
  ...
  if ([ROLES.DEPUTADO, ROLES.EQUIPE].includes(data.role) && process.env.ALLOW_ADMIN_REGISTER !== "true") {
  ...
  return withTransaction(async (client) => {
    ...
    if (data.role === "LIDER" && !rootLeaderId) {
