...
import { makeReferralCodeForRole, makeReferralUrl } from "./referrals.js";
...
export async function listUsers(q = {}) {
  ...
  return { items: result.rows.map(withPublicEmail), page, limit, total: count.rows[0].total };
}
...
export async function ensureReferralLinksForAll(appUrl) {
  ...
  for (const row of result.rows) {
    for (let attempt = 0; attempt < 5; attempt += 1) {
      const code = makeReferralCodeForRole(row.role);
      try {
        await query(
...
export async function getUserProfile(id) {
  ...
  return withPublicEmail(result.rows[0]) || null;
}
...
export async function updateUser(id, input = {}) {
  ...
  if (canLogin(role)) {
    ...
    if (!email) throw new ApiError(400, "Email obrigatorio para equipe.");
    if (input.email !== undefined || input.role !== undefined) setUser("email", validateEmail(email));
  } else if (input.email !== undefined || input.role !== undefined) {
    setUser("email", validateOptionalEmail(input.email) || makeInternalEmail(existing.phone || id));
  }
