...
async function ensureContactMessageSupportSchema() {
  ...
  await query("alter table contact_messages add constraint contact_messages_status_check check (status in ('NOVO', 'LIDO', 'RESPONDIDO'))");
...
export async function replyContactMessage(id, input = {}) {
  await ensureContactMessageSupportSchema();
  const reply = requireString(input.reply || input.resposta, "reply", 2);
  const result = await query(
