...
const REFERRAL_CODE_RE = /^(AG\d{3,}|SIV[A-Z0-9]{6,})$/;
...
export function validateRole(role) {
  const value = String(role || ROLES.CADASTRADOS).toUpperCase();
...
export function normalizeReferralCode(value) {
  ...
  if (!code) return null;
  if (!REFERRAL_CODE_RE.test(code)) throw new ApiError(400, "Codigo de indicacao invalido.");
  return code;
}
...
export function assertValidPhotoUrl(photoUrl, { required = true } = {}) {
  ...
  if (!/^(https?:\/\/|\/|mock:\/\/|data:image\/(jpeg|png|webp);base64,)/i.test(value)) {
